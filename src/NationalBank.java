...
public class NationalBank {
    ...
    void bankInfo(String cityName) {
        boolean bank = true;
        while (bank) {
            int accountOption = 0;
            while (accountOption < 1){
                try {
                    accountOption = Integer.valueOf(starylMain.readLine("Welcome to the National Bank in " + cityName + "!\nWhat would you like to do?\nInput:\n<1> deposit\n<2> withdraw\n<3> leave"));
                }
    ...
    void depositInfo() {
        ...
        while (planOption < 1){
            try {
                planOption = Integer.valueOf(starylMain.readLine("Which interest plan would you like to choose?" +
    ...
    void depositAmount() {
        ...
        do {
            while (amount < 1){
                try {
                    amount = Integer.valueOf(starylMain.readLine("Please input the amount you would like to deposit"));
                }
                catch (NumberFormatException ex) {}
            }
            if (amount >= p.items[2]) {
